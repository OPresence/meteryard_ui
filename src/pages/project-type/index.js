...
const StateComponent = () => {
  ...
  const Create_ProjectType = async (value) => {
    ...
    try {
      setIsLoading(true);
      const res = await PostApiFunction({
        endPoint: Apiconfigs.createProjectType,
        data: {
  ...
  const Update_Country = async (value) => {
    try {
      ...
      const res = await PutApiFunction({
        endPoint: Apiconfigs.editProjectType,
        data: {
          projectType: value?.project_type,
          status: value?.status,
        },
        params: {
          projectTypeId: _viewData?._id,
        },
      });
      if (res) {
        ProjectTypeFunction();
  ...
  const DeleteBanner = async (value) => {
    try {
      ...
      const res = await DeleteApiFunction({
        endPoint: Apiconfigs.deleteProjectType,
        params: {
          projectTypeId: _viewData?._id,
        },
      });
        ...
        <Box className="mainPage">
          <Box mt={1} mb={1}>
            <FilterComponent
              ...
              HeadingDialog="Create Project Type"
              open={open}
