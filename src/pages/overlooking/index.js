...
const StateComponent = () => {
  ...
  const GetCityList = async () => {
    try {
      ...
      const res = await PostApiFunction({
        endPoint: Apiconfigs?.listAllOverlooking,
        data: {
          page: page,
          limit: "10",
        },
  ...
  const Add_Country = async (value) => {
    ...
    try {
      setIsLoading(true);
      const res = await PostApiFunction({
        endPoint: Apiconfigs.createOverlooking,
        data: {
          overlooking: value?.project_type,
          status: value?.status,
        },
      });
      if (res) {
  ...
  const Update_Country = async (value) => {
    try {
      ...
      const res = await PutApiFunction({
        endPoint: Apiconfigs.editOverlooking,
        data: {
          waterResource: value?.project_type,
          status: value?.status,
        },
        params: {
          overlookingId: _viewData?._id,
        },
      });
      if (res) {
        GetCityList();
  ...
  const AD_country = async (value) => {
    try {
      setIsLoading(true);
      const res = await PutApiFunction({
        endPoint: Apiconfigs.activeDeactiveOverlooking,
        params: {
          overlookingId: _viewData?._id,
        },
      });
  ...
  const DeleteBanner = async (value) => {
    try {
      ...
      const res = await DeleteApiFunction({
        endPoint: Apiconfigs.deleteOverlooking,
        params: {
          overlookingId: _viewData?._id,
        },
      });
    ...
    <AdminLayout>
      <Root>
        <Box className="mainPage">
          <Box mt={1} mb={1}>
            <FilterComponent
              title="Overlooking List"
              ButtonName="Create Overlooking"
              HeadingDialog="Create Overlooking"
              open={open}
            ...
            {_listloading ? (
              ...
              <TableList
                data={_bannerlist?.map((data, index) => ({
                  "Overlooking Name": data?.overlooking,
          ...
          {openView && (
            <ViewDialog
              title="Water Overlooking List"
              ButtonName="Update Overlooking"
              HeadingDialog="Update Overlooking"
              _viewData={_viewData}
