...
const ViewOverLooking = ({
  ...
  const formValidationSchemaDepartment = yep.object().shape({
    project_type: yep.string().required("overlooking name is required."),
    status: yep.string().required("status is required."),
  });

  useEffect(() => {
    setInitialState({
      project_type: _viewData?.overlooking,
                ...
                <Grid item lg={6} md={6} sm={12}>
                  <Box mt={2}>
                    <TextField
                      ...
                      label="Overlooking"
                      variant="outlined"
                      placeholder="Enter overlooking name"
                      name="project_type"
...
export default ViewOverLooking;
