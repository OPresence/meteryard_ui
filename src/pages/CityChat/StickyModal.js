          ...
          {!open && (
            <Box sx={{
              ...
              <IconButton
                ...
                sx={{ gap: '5rem', left: '25px', borderRadius: '15px' }}
              >
                <FilterAltIcon />
                <ForumOutlinedIcon/>
               
              </IconButton>
              <ArrowUpwardOutlinedIcon sx={{marginLeft:'12rem'}}/>
            </Box>
          )}
        <Modal open={open} onClose={handleCloseModal}>
      <Slide direction="up" in={open} mountOnEnter unmountOnExit>
        <div style={{ position: 'sticky', bottom: 0, background: '#f0f0f0', padding: '20px'}}>
          <IconButton onClick={expanded ? handleCollapse : handleExpand} style={{ position: 'absolute', top: '10px', right: '10px' }}>
            {expanded ? <KeyboardArrowDown /> : <ArrowUpwardOutlinedIcon />}
          </IconButton>
