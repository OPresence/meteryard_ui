...
const StateComponent = () => {
  ...
  const Add_Country = async (value) => {
    ...
    try {
      setIsLoading(true);
      const res = await PostApiFunction({
        endPoint: Apiconfigs.createPropFacing,
        data: {
          propFacing: value?.project_type,
          status: value?.status,
        },
      });
      if (res) {
  ...
  const Update_Country = async (value) => {
    try {
      ...
      const res = await PutApiFunction({
        endPoint: Apiconfigs.editPropFacing,
        data: {
          propFacing: value?.project_type,
          status: value?.status,
        },
        params: {
          propFacingId: _viewData?._id,
        },
      });
      if (res) {
        GetCityList();
  ...
  const AD_country = async (value) => {
    try {
      setIsLoading(true);
      const res = await PutApiFunction({
        endPoint: Apiconfigs.activeDeactivePropFacing,
        params: {
          propFacingId: _viewData?._id,
        },
      });
  ...
  const DeleteBanner = async (value) => {
    try {
      ...
      const res = await DeleteApiFunction({
        endPoint: Apiconfigs.deletePropFacing,
        params: {
          propFacingId: _viewData?._id,
        },
      });
    ...
    <AdminLayout>
      <Root>
        <Box className="mainPage">
          <Box mt={1} mb={1}>
            <FilterComponent
              title="Property Facing List"
              ButtonName="Create Facing"
              HeadingDialog="Create Property Facing"
              open={open}
            ...
            {_listloading ? (
              ...
              <TableList
                data={_bannerlist?.map((data, index) => ({
                  "Property Facing": data?.propFacing,
          ...
          {openView && (
            <ViewDialog
              ...
              HeadingDialog="Update Property Facing"
              _viewData={_viewData}
