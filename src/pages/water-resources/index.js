...
const StateComponent = () => {
  ...
  const GetCityList = async () => {
    try {
      ...
      const res = await PostApiFunction({
        endPoint: Apiconfigs?.listAllWaterResource,
        data: {
          page: page,
          limit: "10",
        },
  ...
  const Add_Country = async (value) => {
    ...
    try {
      setIsLoading(true);
      const res = await PostApiFunction({
        endPoint: Apiconfigs.createWaterResource,
        data: {
  ...
  const Update_Country = async (value) => {
    try {
      ...
      const res = await PutApiFunction({
        endPoint: Apiconfigs.editWaterResource,
        data: {
          waterResource: value?.project_type,
          status: value?.status,
        },
        params: {
          waterResourceId: _viewData?._id,
        },
      });
      if (res) {
        GetCityList();
  ...
  const AD_country = async (value) => {
    try {
      setIsLoading(true);
      const res = await PutApiFunction({
        endPoint: Apiconfigs.activeDeactiveWaterResource,
        params: {
          waterResourceId: _viewData?._id,
        },
      });
  ...
  const DeleteBanner = async (value) => {
    try {
      ...
      const res = await DeleteApiFunction({
        endPoint: Apiconfigs.deleteWaterResource,
        params: {
          waterResourceId: _viewData?._id,
        },
      });
    ...
    <AdminLayout>
      <Root>
        <Box className="mainPage">
          <Box mt={1} mb={1}>
            <FilterComponent
              title="Water Resource List"
              ButtonName="Create Resource"
              HeadingDialog="Create Resource"
              open={open}
            ...
            {_listloading ? (
              ...
              <TableList
                data={_bannerlist?.map((data, index) => ({
                  "Water Resource": data?.waterResource,
