...
const StateComponent = () => {
  ...
  const GetCityList = async () => {
    try {
      ...
      const res = await PostApiFunction({
        endPoint: Apiconfigs?.listAllPropertyAvailibility,
        data: {
          page: page,
          limit: "10",
        },
  ...
  const Add_Country = async (value) => {
    ...
    try {
      setIsLoading(true);
      const res = await PostApiFunction({
        endPoint: Apiconfigs.createPropertyAvailibility,
        data: {
          propertyAvailibility: value?.project_type,
          status: value?.status,
        },
      });
      if (res) {
  ...
  const Update_Country = async (value) => {
    try {
      ...
      const res = await PutApiFunction({
        endPoint: Apiconfigs.editPropertyAvailibility,
        data: {
          propertyAvailibility: value?.project_type,
          status: value?.status,
        },
        params: {
          propAvailibilityId: _viewData?._id,
        },
      });
      if (res) {
        GetCityList();
  ...
  const AD_country = async (value) => {
    try {
      setIsLoading(true);
      const res = await PutApiFunction({
        endPoint: Apiconfigs.activeDeactivePropertyAvailibility,
        params: {
          propAvailibilityId: _viewData?._id,
        },
      });
  ...
  const DeleteBanner = async (value) => {
    try {
      ...
      const res = await DeleteApiFunction({
        endPoint: Apiconfigs.deletePropertyAvailibility,
        params: {
          propAvailibilityId: _viewData?._id,
        },
      });
    ...
    <AdminLayout>
      <Root>
        <Box className="mainPage">
          <Box mt={1} mb={1}>
            <FilterComponent
              title="Property Availability List"
              ButtonName="Create Property"
              HeadingDialog="Update Property"
              open={open}
            ...
            {_listloading ? (
              ...
              <TableList
                data={_bannerlist?.map((data, index) => ({
                  "Property Availability": data?.propertyAvailibility,
