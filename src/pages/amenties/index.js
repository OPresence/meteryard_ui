...
const StateComponent = () => {
  ...
  const GetCityList = async () => {
    try {
      ...
      const res = await PostApiFunction({
        endPoint: Apiconfigs?.listAllAminity,
        data: {
          page: page,
          limit: "10",
        },
  ...
  const Add_Country = async (value) => {
    try {
      setIsLoading(true);
      const res = await PostApiFunction({
        endPoint: Apiconfigs.createAminity,
        data: {
          aminity: value?.project_type,
          status: value?.status,
        },
      });
      if (res) {
  ...
  const Update_Country = async (value) => {
    try {
      ...
      const res = await PutApiFunction({
        endPoint: Apiconfigs.editAminity,
        data: {
          aminity: value?.project_type,
          status: value?.status,
        },
        params: {
          aminityId: _viewData?._id,
        },
      });
      if (res) {
        GetCityList();
  ...
  const AD_country = async (value) => {
    try {
      setIsLoading(true);
      const res = await PutApiFunction({
        endPoint: Apiconfigs.activeDeactiveAminity,
        params: {
          aminityId: _viewData?._id,
        },
      });
  ...
  const DeleteBanner = async (value) => {
    try {
      ...
      const res = await DeleteApiFunction({
        endPoint: Apiconfigs.deleteAminity,
        params: {
          aminityId: _viewData?._id,
        },
      });
