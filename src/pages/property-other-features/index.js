  ...
  const GetCityList = async () => {
    try {
      ...
      const res = await PostApiFunction({
        endPoint: Apiconfigs?.listAllOtherFeature,
        data: {
          page: page,
          limit: "10",
        },
  ...
  const Update_Country = async (value) => {
    try {
      ...
      const res = await PutApiFunction({
        endPoint: Apiconfigs.editOtherFeature,
        data: {
          waterResource: value?.project_type,
          status: value?.status,
        },
        params: {
          propOtherFeatureId: _viewData?._id,
        },
      });
      if (res) {
        GetCityList();
  ...
  const DeleteBanner = async (value) => {
    try {
      ...
      const res = await DeleteApiFunction({
        endPoint: Apiconfigs.deleteOtherFeature,
        params: {
          propOtherFeatureId: _viewData?._id,
        },
      });
    ...
    <AdminLayout>
      <Root>
        <Box className="mainPage">
          <Box mt={1} mb={1}>
            <FilterComponent
              title="Other Features List"
              ButtonName="Create Features"
              HeadingDialog="Create Other Features"
              open={open}
            ...
            {_listloading ? (
              ...
              <TableList
                data={_bannerlist?.map((data, index) => ({
                  "Other Features": data?.propOtherFeature,
          ...
          {openView && (
            <ViewDialog
              ...
              HeadingDialog="Update Other Features"
              _viewData={_viewData}
